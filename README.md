...
- LLM：
  - macOS：`mlx-community/Qwen2.5-7B-Instruct-4bit`，安装到 `MODEL_HOME/llm/qwen2.5-7b-instruct-4bit-mlx`。该本地运行时依赖 MLX，仅支持 Apple Silicon macOS。
  - macOS 轻量模型：`mlx-community/Qwen2.5-3B-Instruct-4bit`，安装到 `MODEL_HOME/llm/qwen2.5-3b-instruct-4bit-mlx`。适合标题、润色和短口播文案生成。
  - Windows：`Qwen/Qwen2.5-7B-Instruct-GGUF` 的 `Q4_K_M` 量化版本，安装到 `MODEL_HOME/llm/qwen2.5-7b-instruct-q4-k-m-gguf`，运行时使用 `llama.cpp`。
- ASR：`Qwen/Qwen3-ASR-1.7B`，安装到 `MODEL_HOME/asr/qwen3-asr-1.7b`
- TTS：`openbmb/VoxCPM2`，安装到 `MODEL_HOME/tts/voxcpm2`
...
- macOS：`MODEL_HOME/llm/qwen2.5-7b-instruct-4bit-mlx`
- macOS：`MODEL_HOME/llm/qwen2.5-3b-instruct-4bit-mlx`
- Windows：`MODEL_HOME/llm/qwen2.5-7b-instruct-q4-k-m-gguf`
- `MODEL_HOME/asr/qwen3-asr-1.7b`
- `MODEL_HOME/tts/voxcpm2`
...
MODEL_HOME/llm/qwen2.5-3b-instruct-4bit-mlx/
├── config.json
└── tokenizer.json

MODEL_HOME/llm/qwen2.5-7b-instruct-q4-k-m-gguf/
└── *.gguf
...
模型会按平台隔离注册：macOS 只展示 MLX 文本模型，Windows 只展示 GGUF 文本模型，避免用户看到当前系统不可用的本地模型。Windows 离线包还需要内置 `runtime/llama.cpp/llama-cli.exe`，或通过 `DH_LLAMA_CPP_BIN` 指定可执行文件路径；缺失时模型状态会显示为运行时不完整。
...
- `MODEL_PLAZA_TTS_MODEL=voxcpm2`：指定 TTS 模型 ID。
