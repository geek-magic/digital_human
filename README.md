...
长视频会自动分段调用 MuseTalk：默认每段 60 秒，逐段生成后再用 FFmpeg 拼接，避免一次性处理长视频导致临时帧、坐标文件和进程时间过大。可通过 `AVATAR_SEGMENT_SECONDS` 调整分段长度，允许范围为 10-120 秒；单段 MuseTalk Adapter 仍会受 `MUSETALK_TIMEOUT_MS` 控制。
